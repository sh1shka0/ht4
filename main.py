...
# ОБЯЗАТЕЛЬНО https://phoenixnap.com/kb/ffmpeg-windows и перезагрузить комп ещё
...
def default_markup():
    ...
    btn_graph = types.KeyboardButton('Просмотреть график продуктивности')
...
def task_priority(message, name, type_):
    ...
    if priority == 'Отмена':
        bot.send_message(message.chat.id, "Добавление задачи отменено", reply_markup=default_markup())
        return
    msg=bot.send_message(message.chat.id,"Когда вам напомнить об этой задаче (дедлайн)? Напишите 'нет', чтобы создать задачу без напоминания.",
...
@bot.message_handler(func= lambda x:  x.text=='Просмотреть задачи')
def mark_task(message):
...
@bot.message_handler(func= lambda x:  x.text=='Просмотреть график продуктивности')
